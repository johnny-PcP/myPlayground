...
test("應該正確設定 maxDistanceMultiple", async () => {
  const wrapper = mount(BtnNaughty, {
...
test("當按鈕被 disabled 時，應該觸發 run 事件並移動", async () => {
  const wrapper = mount(BtnNaughty);
...
test("應該正確渲染 default slot 的按鈕內容", async () => {
  const wrapper = mount(BtnNaughty, {
    slots: {
...
test("應該正確渲染 rubbing slot 的拓印內容", async () => {
  const wrapper = mount(BtnNaughty, {
    slots: {
